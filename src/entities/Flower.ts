...
export class Flower implements ICharacter {
  ...
  private createHead(): THREE.Group {
    ...
    const petals = [
      ...
      { angle: 130, length: 0.18, width: 0.30 },  // 5 - medium-narrow
      { angle: 155, length: 0.21, width: 0.40 },  // 6 - wide
      { angle: 180, length: 0.18, width: 0.38 },  // 7 - wide
      { angle: 205, length: 0.16, width: 0.34 },  // 8 - medium
      { angle: 235, length: 0.10, width: 0.28 },  // 9 - medium-narrow
      { angle: 265, length: 0.07, width: 0.24 },  // 10 - narrow (tiny petal)
      { angle: 295, length: 0.10, width: 0.28 },  // 11 - medium-narrow
    ...
    const startAngle = (firstPetal.angle - firstPetal.width * 0.5 * (180 / Math.PI)) * (Math.PI / 180)
    flowerShape.moveTo(
    ...
    for (let i = 0; i < petals.length; i++) {
      ...
      if (nextAngleRad < angleRad) valleyAngle = angleRad + (nextAngleRad + Math.PI * 2 - angleRad) / 2
